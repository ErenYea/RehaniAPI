...
const fillFormNew = async (data) => {
  ...
  for (let i = 0; i < textFields.length; i++) {
    const textField = form.getField(textFields[i].name);
    if (textField) {
      ...
      widgets.forEach((w) => {
        ...
        if (textFields[i].name in data.signatureFields) {
          props.push({
            page: pageIndex + 1,
